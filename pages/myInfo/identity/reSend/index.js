// pages/myInfo/identity/reSend/index.js
...
Page({
  ...
  async onLoad(options) {
    ...
    const [res_identity, err_identity] = await to(
      userApi.getUserIdentityDetail(options.id)
    );
    const [res, err] = await to(
      organizationApi.getOrganizations(organization_search_key, 0, 10)
    );
    ...
    this.setData({
      attachments: res_identity.data.data.attachments.map((item) => {
        return {
          ...item,
          url: item.link,
        };
      }),
      name: res_identity.data.data.identity_info.name,
      description: res_identity.data.data.identity_info.description,
      organization: res_identity.data.data.organization_info,
      organizations: res.data.data,
    });
    wx.hideLoading();
  },
