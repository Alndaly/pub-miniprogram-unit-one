  ...
  methods: {
    async onVoteComment(e) {
      ...
      this.setData({
        ...
        "commentItem.vote": _this.data.commentItem.is_vote
          ? _this.data.commentItem.vote - 1
          : _this.data.commentItem.vote + 1,
      });
      const comment = e.currentTarget.dataset.comment;
      const [res, err] = await to(
        activityApi.voteComment(comment.id, this.data.commentItem.is_vote)
      );
      if (err) {
        this.setData({
          "commentItem.is_vote": !_this.data.commentItem.is_vote,
    ...
    onTapComment(e) {
      let myEventDetail = this.data.commentItem; // detail对象，提供给事件监听函数
